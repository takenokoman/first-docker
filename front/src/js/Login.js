...
export default class Login extends React.Component {
  ...
  handleSubmit(e) {
    ...
    axios
      ...
      .then((res) => {
        alert(res.data.msg + 'ユーザー名: ' + res.data.userName);
        console.log(res.data.userName + "さんがログインしました。ログイン状態:" + this.state.login
        + res.data.userId);
        this.props.handleLogin(res.data.userName, res.data.userId);
      })
      .then((res) => {
        console.log("おしずかにぃ！");
        this.setState({login: true});
        const tl = this.props.toggleLogin.bind(this);
        tl();
      })
      .catch((error) => {
        alert('ログインに失敗しました');
